...
![Atlantic salmon migration coloured by adaptive margin](docs/wiki/figures/salmon_migration_map.png)

> *The framework's output is the **adaptive margin** `A_t`. Here an anadromous salmon's migration path
> is coloured by the margin it **encounters** (a), over the exposure field it traverses across space
> and time (b) — capacity is stage-resolved, exposure is occupancy-weighted, and at spawning the fish
> is *compliant yet eroded*. Regenerate: `examples/salmon_migration_map.jl`.*
...
- Update the relevant [`docs/wiki/`](docs/wiki/Home.md) page in the same PR as any
  ...
  current.
- Figures are regenerated by their scripts (`examples/salmon_migration_map.jl` (landing),
  `salmon_migration_figure.jl`, `wiki_figures.jl`, `validation_forest_plot.jl`) — edit the script,
  not the images.
