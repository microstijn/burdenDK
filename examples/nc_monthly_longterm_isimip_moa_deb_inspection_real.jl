...
# Spatial subset. For a Rhine-Meuse / Dutch delta test, this is a reasonable start.
# Widen or tighten as needed.
    ...
    try
        ...
        for d in dims
            ...
            elseif occursin("time", dl)
                ...
                # Keep all values for unknown dimensions. Most DynQual files should not need this.
                push!(idxs, Colon())
                push!(kept_dims, d)
            end
        end
        ...
        # If a singleton dimension remains, drop it.
        ...
        # Standardize to lon x lat orientation for plotting and consistency.
...
function load_raw_layers_for_time(year, month)
    ...
    BOD, lon2, lat2, BOD_var = if use_BODload_instead_of_organic
    ...
    FC, lon3, lat3, FC_var = load_nc_layer_auto(FC_file, FC_var_candidates, t_idx, bbox)
    TDS, lon4, lat4, TDS_var = load_nc_layer_auto(TDS_file, TDS_var_candidates, t_idx, bbox)
