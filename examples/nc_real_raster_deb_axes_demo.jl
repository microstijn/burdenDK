...
# Note: In a real environment, you would run this with:
# using NCDatasets
# However, this example script is intended to be self-contained for the repository demo without needing NCDatasets to be globally available in scope for simple ASCII conversions if we dummy the loader, but here we provide the full code logic.
...
function run_nc_real_raster_deb_axes_demo(;
    ...
    pathogen_norm = normalise_layer(pathogen_raw)
    organic_norm = normalise_layer(organic_raw)
