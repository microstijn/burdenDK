...
- Figures live in [`docs/wiki/figures/`](figures/) and are regenerated by their scripts — edit the
  script, not the image: `examples/framework_margin_map.jl` (landing hero),
  `examples/wiki_figures.jl` (mechanism), `examples/validation_forest_plot.jl` (forest plot),
  `examples/salmon_migration_figure.jl` and `examples/salmon_migration_map.jl` (life-stage + movement).
