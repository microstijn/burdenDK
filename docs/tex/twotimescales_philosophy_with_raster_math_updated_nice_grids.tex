...
% -----------------------------------------------------------------------------
% Synthetic raster drawing macros used in the mathematical section.
% They are intentionally small and self-contained, so the .tex file remains portable.
% -----------------------------------------------------------------------------
...
\end{tikzpicture}}
\newcommand{\drawRasterBurden}{%
\begin{tikzpicture}[scale=0.58]
\cellbox{loworange}{(0,0)} \cellbox{midorange}{(0.8,0)} \cellbox{midorange}{(1.6,0)} \cellbox{highorange}{(2.4,0)} \cellbox{highorange}{(3.2,0)}
\cellbox{loworange}{(0,0.8)} \cellbox{midorange}{(0.8,0.8)} \cellbox{highorange}{(1.6,0.8)} \cellbox{highorange}{(2.4,0.8)} \cellbox{midorange}{(3.2,0.8)}
\cellbox{loworange}{(0,1.6)} \cellbox{midorange}{(0.8,1.6)} \cellbox{highorange}{(1.6,1.6)} \cellbox{midorange}{(2.4,1.6)} \cellbox{midorange}{(3.2,1.6)}
\cellbox{loworange}{(0,2.4)} \cellbox{midorange}{(0.8,2.4)} \cellbox{midorange}{(1.6,2.4)} \cellbox{midorange}{(2.4,2.4)} \cellbox{loworange}{(3.2,2.4)}
\cellbox{loworange}{(0,3.2)} \cellbox{loworange}{(0.8,3.2)} \cellbox{loworange}{(1.6,3.2)} \cellbox{loworange}{(2.4,3.2)} \cellbox{loworange}{(3.2,3.2)}
\gridframe\rastlabel{$B_{1,t}$: retained burden}
\end{tikzpicture}}
...
\gridframe\rastlabel{$E_M^G$: maintenance impairment}
\end{tikzpicture}}
...
\gridframe\rastlabel{$F_{0.95}^G$: feature raster}
\end{tikzpicture}}
...
\gridframe\rastlabel{cluster raster $R(g)$}
\end{tikzpicture}}
...
The mathematical formulation does not simply operationalise the conceptual sequence; it interprets it. Each modelling step---defining exposure, assigning stressor pressure, aggregating physiological burden, representing adaptive margin, and computing amplification---commits the framework to a particular account of how organisms relate to their milieu. This section updates the formal chain to reflect the current implementation: chemical memory is implemented as retained burden, mixture effects are represented through axis-level alternatives including grouped concentration addition followed by independent action, and spatial vulnerability work now proceeds through threshold-free feature vectors before any clustering or raster output.

\subsection{Spatial concentration matrices}

Let $g=(x,y)$ denote a grid cell, $t$ a slow background time index, $j$ a compound, and $q$ a species, population, life stage, or target biological profile. A concentration raster for compound $j$ at time $t$ is
\begin{equation}
  \mathbf{C}_{j,t}\in \mathbb{R}_{\ge 0}^{n_y\times n_x},
\end{equation}
with entry $C_{j,t}(g)$. A multi-compound water-quality scenario is therefore a stack of concentration rasters. Figure-like raster sketches below show three possible deterministic spatial patterns: a river or plume, diffuse runoff, and an urban hotspot. They are illustrative, but they show the intended mathematical object.
...
The model does not cluster these concentration rasters directly. Concentrations are first transformed into retained burden, active stress, axis impairment, weighted species response, adaptive margin, restoring force, and amplification.

\subsection{Target-relative exposure and chemical memory}

In the general runtime pipeline, raw environmental stressors can first be filtered by target-specific contact, habitat, or uptake modifiers,
\begin{equation}
  \mathbf{e}^{(q)}_{g,t}=\mathbf{H}^{(q)}\mathbf{R}_{g,t},
\end{equation}
where $\mathbf{H}^{(q)}$ encodes how the milieu is encountered by the target. In the ECOTOX-driven chemical workflow, the relevant exposure field is a compound concentration raster $C_{j,t}(g)$ or a retained internal burden raster $B_{j,t}(g)$.

Compound memory is represented by
\begin{equation}
  B_{j,t}(g)=\rho_j B_{j,t-1}(g)+(1-
ho_j)K_j C_{j,t}(g),
  \label{eq:compound_memory_updated}
\end{equation}
where $\rho_j\in[0,1)$ is monthly retention or carryover, and $K_j\geq 0$ is a bioaccumulation or internal magnification factor. The limiting case $\rho_j=0$ and $K_j=1$ recovers the stateless relation $B_{j,t}=C_{j,t}$. Chemical memory $B_{j,t}$ is not DEBtox scaled damage and is distinct from any future organism-level physiological condition memory $Z_t$.

\begin{center}
\begin{tabular}{cc}
\drawRasterCOne & \drawRasterBurden\\
\end{tabular}
\end{center}

The second raster above illustrates why memory matters: the spatial footprint used for effect comparison may be smoother, delayed, or magnified relative to the ambient concentration field.

\subsection{Active stress and routing to DEB-informed axes}

For contaminant $j$, active stress is computed from the effective concentration used for ECOTOX comparison,
\begin{equation}
  x_{j,t}(g)=\max\left(0,\frac{E_{j,t}(g)-NOEC_j}{EC50_j-NOEC_j}\right),
  \qquad EC50_j>NOEC_j,
  \label{eq:ecotox_scaling_updated}
\end{equation}
where $E_{j,t}=C_{j,t}$ in stateless calculations and $E_{j,t}=B_{j,t}$ in stateful memory calculations. The active stress $x_{j,t}$ is not clamped at one; exposure above $EC50$ can produce $x>1$.

Each ECOTOX effect code is routed to one DEB-informed physiological axis
\begin{equation}
  a\in\mathcal{A}=\{\mathrm{assimilation},\mathrm{maintenance},\mathrm{growth},\mathrm{reproduction}\}.
\end{equation}
Let $r(j)$ be the routed axis for record $j$. Then
\begin{equation}
  x_{j,a,t}(g)=x_{j,t}(g)\,\mathbb{I}\{r(j)=a\}.
\end{equation}
These axes are DEB-informed process axes. The framework does not claim that $E_a$, $Q$, or $A$ are standard DEB state variables, and it does not implement reserve, structure, maturity, mobilisation, $\kappa$ allocation, DEBkiss growth/reproduction ODEs, starvation, survival, or DEBtox scaled damage.

\subsection{Axis-level mixture-effect models}

The current implementation includes three axis-level mixture-effect assumptions. They should be described as mixture-effect assumptions, not as known compound interactions.

For toxic-unit summation,
\begin{equation}
  X_{a,t}(g)=\sum_j x_{j,a,t}(g),
  \qquad
  E_{a,t}^{TU}(g)=\frac{X_{a,t}(g)}{1+X_{a,t}(g)}.
\end{equation}
For independent action,
\begin{equation}
  E_{j,a,t}(g)=\frac{x_{j,a,t}(g)}{1+x_{j,a,t}(g)},
  \qquad
  E_{a,t}^{IA}(g)=1-\prod_{j:r(j)=a}\left(1-E_{j,a,t}(g)\right).
\end{equation}
For grouped concentration addition followed by independent action, let $h(j)$ be an empirical effect group, currently represented by an ECOTOX effect code. Within each axis--group combination,
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:grouped_ca_ia}
\end{equation}
The grouped model is the preferred default where effect-code grouping is available; toxic-unit summation and independent action are retained as sensitivity alternatives. The following raster sketch shows an example of the resulting maintenance-axis impairment under the grouped model.

\begin{center}
\drawRasterAxis
\end{center}

\subsection{Species or archetype response}

For species or archetype $q$, let $w_{q,a}$ be normalised axis weights with $\sum_a w_{q,a}=1$. The weighted impairment is
\begin{equation}
  Q_{q,t}^{m}(g)=\sum_{a\in\mathcal{A}}w_{q,a}E_{a,t}^{m}(g),
  \label{eq:Q_updated}
\end{equation}
where $m\in\{TU,IA,G\}$. The current EC50-anchored response path uses
\begin{equation}
  A_{q,t}^{m}(g)=A_{0,q}\max\left(10^{-6},1-Q_{q,t}^{m}(g)\right).
  \label{eq:A_updated}
\end{equation}
It is often useful to express the continuous relative margin remaining,
\begin{equation}
  R^A_{q,t,m}(g)=\frac{A_{q,t}^{m}(g)}{A_{0,q}}
  =\max\left(10^{-6},1-Q_{q,t}^{m}(g)\right).
\end{equation}
The corresponding margin depletion is
\begin{equation}
  D^A_{q,t,m}(g)=1-R^A_{q,t,m}(g),
\end{equation}
which is a model-derived continuous quantity, not an external threshold.

Adaptive margin determines restoring force,
\begin{equation}
  \lambda_q(A)=\lambda_{\min,q}+\left(\lambda_{\max,q}-\lambda_{\min,q}\right)\frac{A_+}{K_{A,q}+A_+},
  \qquad A_+=\max(A,0),
  \label{eq:lambda_updated}
\end{equation}
and amplification,
\begin{equation}
...
\subsection{Fast event burden and amplification}

Let $C_{\mathrm{event}}(\tau)$ be an acute event profile on the fast time variable $\tau$, and let $y(\tau)$ denote the displacement or burden induced by that event. For a fixed background state $A_{q,t}^{m}(g)$,
\begin{equation}
  \frac{dy}{d\tau}=-\lambda_q(A_{q,t}^{m}(g))y+\beta C_{\mathrm{event}}(\tau),
  \qquad y(0)=0.
\end{equation}
Integrating over time gives
\begin{equation}
  \int_0^\infty y(\tau)\,d\tau
  =\frac{\beta}{\lambda_q(A_{q,t}^{m}(g))}\int_0^\infty C_{\mathrm{event}}(\tau)\,d\tau,
\end{equation}
provided the event is integrable and $\lambda>0$. Thus the integrated burden of the same acute event scales inversely with the background-conditioned restoring force, yielding Equation~\eqref{eq:F_updated}.

\subsection{Threshold-free spatial feature vectors}

The spatial vulnerability pipeline now treats feature construction as a core mathematical layer. For each cell $g$, the response arrays are summarised into a threshold-free feature vector $\bm z(g)$. The grouped model $G$ supplies the preferred main response features; $TU$ and $IA$ supply continuous sensitivity deltas.

A compact feature vector includes
\begin{equation}
\bm z(g)=
\begin{bmatrix}
\overline{Q}^{G}(g)\\
...
\end{bmatrix}.
\end{equation}
Here quantiles, means, maxima, standard deviations, margin summaries, mixture-model deltas, axis entropy, and seasonal timing are continuous summaries of model output. No feature is defined as a fraction above an arbitrary threshold such as $F>1.10$ or $Q>0.80$.
...
\end{equation}
If all axis impairments are zero, $H_E(g)=0$ and the dominant axis code is set to zero. A feature raster such as $F_{0.95}^{G}(g)$ can be written back to the spatial grid:

\begin{center}
\drawRasterFeature
\end{center}

\subsection{Standardisation, feature dropping, and regime clustering}

Before clustering, feature columns are standardised,
\begin{equation}
  \widetilde{Z}_{g,p}=\frac{Z_{g,p}-\mu_p}{\sigma_p},
\end{equation}
where $Z_{g,p}$ is the $p$th feature in cell $g$. Non-finite columns and zero-variance columns are dropped with recorded reasons; highly correlated columns may optionally be dropped. This preprocessing is not a biological thresholding step: it creates no new vulnerability variables and no exceedance features.

Clustering can then be performed on the standardised continuous feature matrix,
\begin{equation}
  \min_{\mathcal{C}_1,\ldots,\mathcal{C}_K}\sum_{k=1}^{K}\sum_{g\in\mathcal{C}_k}\left\|\widetilde{\bm z}(g)-\bm\mu_k\right\|_2^2.
\end{equation}
The output is a single categorical raster of vulnerability regimes,
\begin{equation}
  R(g)\in\{1,\ldots,K\},
\end{equation}
where each cluster summarises the combined response pattern of the compound mixture across the selected species or archetypes, months, axes, and mixture-effect assumptions.

\begin{center}
\drawClusterRaster
\end{center}

The cluster raster should always be interpreted together with centroid tables and supporting continuous rasters such as $F_{0.95}^G$, $Q_{0.95}^G$, $H_E$, dominant-axis code, mixture sensitivity, and month of maximum upper-envelope response.
...
Current runtime aggregation is axis-wise after endpoint routing and now includes three explicit mixture-effect assumptions: toxic-unit summation, independent action, and grouped concentration addition followed by independent action. The grouped model is currently the preferred default when effect-code grouping is available, while the other models are retained for sensitivity analysis. These are not fitted interaction models and do not implement synergism, antagonism, or chemical-specific interaction coefficients. They are transparent assumptions for combining compound-level burdens on DEB-informed axes.

The framework also now supports a threshold-free spatial feature-vector layer downstream of model response arrays. This layer converts gridded $Q$, $F$, optional $A/A_0$, and axis-impairment arrays into cell-level continuous features. A subsequent standardisation layer drops non-finite, zero-variance, and optionally redundant features before clustering. Raster and NetCDF integration are therefore downstream of the model and feature-vector mathematics rather than embedded in the biological response functions themselves.
...
For spatial applications, the headline output need not be a separate heatmap for every species. A useful spatial product is a single categorical vulnerability-regime raster whose cluster assignments are derived from threshold-free continuous summaries of the combined compound-mixture response across selected species or archetypes, months, physiological axes, and mixture-effect assumptions. The cluster raster is not a statement that a grid cell is safe or unsafe. It is a regime map: a compact representation of the multivariate vulnerability signature of that cell.

\section{Limitations and Future Work}
\label{sec:limitations}

Several limitations follow directly from the modelling choices above. First, adaptive margin is scalar in the present formulation, even though biological normativity is multidimensional. The scalar representation is useful for operational vulnerability assessment, but it inevitably compresses heterogeneous forms of capacity into a common metric. Second, ECOTOX effect-code grouping is an empirical proxy, not a confirmed mechanistic mode-of-action ontology. The grouped CA-then-IA model should therefore be treated as a defensible default assumption, not as a proven interaction model. Third, chemical memory $B_t$ is included, but organism-level physiological condition memory $Z_t$ remains optional and is not yet the central empirical state variable. Fourth, the framework does not yet implement DEBtox scaled damage, GUTS survival, starvation, or full DEBkiss growth and reproduction dynamics. Finally, social, infrastructural, behavioural, and institutional determinants of exposure and recovery are not explicitly modelled, even though they are crucial for human water--health applications.

Future work should proceed in stages. The first priority is to complete and validate the threshold-free feature-vector and standardisation layers using small deterministic synthetic arrays. The second is deterministic clustering of those feature vectors into vulnerability regimes. The third is spatial output: feature rasters, cluster rasters, NetCDF schemas, and GIS-facing products. The fourth is carefully scoped raster integration using actual gridded environmental data. Physiological condition memory and DEBtox-like scaled damage should remain separate future extensions rather than being conflated with the current chemical memory state.
